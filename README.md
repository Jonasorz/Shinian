...
- **移动端与提醒**：Android/iOS 窄屏共享底部导航与即时切换反馈、PC 任务页内提醒、重复任务自动生成下一实例。
- **图片与备份**：Memo 图片客户端直传、失败持久重试、缩略图与懒加载、私有 Vercel Blob、Neon 时间点恢复及每日完整 ZIP 归档。
...
- **图片附件**：图片直接上传至私有 Blob；上传失败时笔记正文仍会保留，页面会持续显示失败图片并提供“重试上传”。时间线首屏加载 30 条，更多内容按游标继续加载。
...
- 图片由浏览器直接上传至 Private Vercel Blob，避免经过 Vercel Function 请求体；列表优先加载缩略图并延迟加载屏幕外图片。
